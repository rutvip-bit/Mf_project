...
def process_investor_master(cams=None, kfin=None):
    ...
    if existing.empty:
        ...
        ignore = {"created_at", "updated_at", "flag"}

        compare_cols = [c for c in df.columns if c in existing.columns and c not in ignore]

        df_cmp = df[compare_cols].astype(str).fillna("")
        ex_cmp = existing[compare_cols].astype(str).fillna("")

        # IMPORTANT FIX: row-wise exact match check
        existing_set = set(ex_cmp.apply(lambda x: "|".join(x.values), axis=1))

        df["flag"] = df_cmp.apply(lambda x: "|".join(x.values), axis=1).isin(existing_set).astype(int)

        # if DB does not have source column → remove safely
